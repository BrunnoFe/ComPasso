...
def test_set_volume_clamps_above_100(mocker):
    mocker.patch("src.core.audio.platform.system", return_value="Darwin")
    run = mocker.patch("src.core.audio.subprocess.run")
...
def test_set_volume_windows_uses_pycaw(mocker):
    mocker.patch("src.core.audio.platform.system", return_value="Windows")
    fake_device = mocker.MagicMock()
    mocker.patch("src.core.audio.AudioUtilities.GetSpeakers", return_value=fake_device)
...
def test_set_volume_unsupported_os_returns_false(mocker):
    mocker.patch("src.core.audio.platform.system", return_value="Plan9")
    assert audio.set_system_volume(50) is False


def test_set_volume_returns_false_on_exception(mocker):
    mocker.patch("src.core.audio.platform.system", return_value="Linux")
    mocker.patch("src.core.audio.subprocess.run", side_effect=OSError("amixer ausente"))
    assert audio.set_system_volume(50) is False
...
def test_get_volume_macos_parses_output(mocker):
    ...
    mocker.patch("src.core.audio.subprocess.run", return_value=result)
    assert audio.get_system_volume() == pytest.approx(42.0)


def test_get_volume_falls_back_to_50_on_error(mocker):
    mocker.patch("src.core.audio.platform.system", return_value="Linux")
    mocker.patch("src.core.audio.subprocess.run", side_effect=OSError("erro"))

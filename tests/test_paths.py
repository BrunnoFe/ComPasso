...
def test_get_logs_dir_structure(mocker, tmp_path):
    ...
    mocker.patch("compasso.utils.paths.get_app_data_dir", return_value=appdata)
...
def test_ensure_app_dirs_is_idempotent(mocker, tmp_path):
    mocker.patch("compasso.utils.paths.get_documents_dir", return_value=tmp_path / "D")
    mocker.patch("compasso.utils.paths.get_app_data_dir", return_value=tmp_path / "A")
